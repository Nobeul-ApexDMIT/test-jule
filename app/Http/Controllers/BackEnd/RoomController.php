...
class RoomController extends Controller
{
  ...
  {
    RoomCategory::find($request->category_id)->update(['is_featured' => $request->is_featured]);
  ...
  public function storeCategory(Request $request, $language)
  {
    ...
    if($request->filled('image')){
      ...
      $featuredDir = public_path('assets/img/rooms/category/');
    ...
    RoomCategory::create($request->except('language_id','image') + [
      'language_id' => $lang->id,
      'image' => $request->filled('image') ? $featuredImgName : null
    ]);
  ...
  public function updateCategory(Request $request)
  {
    ...
    if($request->filled('image')){
      ...
      @unlink(public_path('assets/img/rooms/category/') . $roomCategory->image);
      $featuredDir = public_path('assets/img/rooms/category/');
      ...
      @copy(public_path($featuredImgURL), $featuredDir . $featuredImgName);
      $roomCategory->image = $request->filled('image') ? $featuredImgName : $roomCategory->image;
    }
    $roomCategory->update();
  ...
  public function deleteCategory(Request $request)
  {
    ...
    if(file_exists(public_path('assets/img/rooms/category/') . $roomCategory->image)){
      @unlink(public_path('assets/img/rooms/category/') . $roomCategory->image);
    }
  ...
  public function storeRoom(Request $request)
  {
    ...
    $sliderImgs = [];
    $sliderDir = public_path('assets/img/rooms/slider_images/');
    ...
    // set a name for the featured image and store it to local storage
    $featuredImgName = time() . '.' . $featuredImgExt;
    $featuredDir = public_path('assets/img/rooms/');
  ...
  public function getSliderImages($id)
  {
    ...
    foreach ($sliderImages as $key => $sliderImage) {
      $data = asset('assets/img/rooms/slider_images') . '/' . $sliderImage;
  ...
  public function updateRoom(Request $request, $id)
  {
    ...
    if (!empty($roomSldImgs)) {
      foreach ($roomSldImgs as $key => $roomSldImg) {
        if (file_exists(public_path('assets/img/rooms/slider_images/') . $roomSldImg)) {
          unlink(public_path('assets/img/rooms/slider_images/') . $roomSldImg);
        }
      }
    }
    ...
    if ($request->filled('featured_img')) {
      ...
      if (
        ...
        file_exists(public_path('assets/img/rooms/') . $room->featured_img)
      ) {
