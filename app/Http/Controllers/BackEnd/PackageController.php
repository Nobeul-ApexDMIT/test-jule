...
class PackageController extends Controller
{
 ...
 public function storeCategory(Request $request, $language)
 {
    ...
    if($request->filled('featured_img')){
      ...
      $featuredDir = public_path('assets/img/packages/category/');
  
      if (!file_exists($featuredDir)) {
      mkdir($featuredDir, 0777, true);
      }
  
      copy(public_path($featuredImgURL), $featuredDir . $featuredImgName);
    }
    ...
    PackageCategory::create($request->except('language_id','featured_img') + [
    'language_id' => $lang->id,
    'featured_img' => $request->filled('featured_img') ? $featuredImgName : null
    ]);
  ...
  public function updateCategory(Request $request)
  {
    ...
    $fileExtension = pathinfo($featuredImgURL, PATHINFO_EXTENSION);
    $featuredImgName = time() . '.' . $fileExtension;
    ...
    if($request->filled('featured_img')){
      ...
      @unlink(public_path('assets/img/packages/category/') . $packageCategory->featured_img);
      $featuredDir = public_path('assets/img/packages/category/');
      ...
      @copy(public_path($featuredImgURL), $featuredDir . $featuredImgName);
      $packageCategory->featured_img = $request->filled('featured_img') ? $featuredImgName : $packageCategory->featured_img;
    }
  ...
  public function deleteCategory(Request $request)
  {
    ...
    if(file_exists(public_path('assets/img/packages/category/') . $packageCategory->featured_img)){
      @unlink(public_path('assets/img/packages/category/') . $packageCategory->featured_img);
    }
  ...
  {
    PackageCategory::find($request->category_id)->update(['is_featured' => $request->is_featured]);
  ...
  public function storePackage(Request $request)
  {
    ...
    foreach ($languages as $language) {
      ...
      if ($settings->package_category_status == 1 && $language->name=='English') {
    ...
    $sliderImgs = [];
    $sliderDir = public_path('assets/img/packages/slider_images/');
    ...
    foreach ($sliderImgURLs as $sliderImgURL) {
      ...
      copy(public_path($sliderImgURL), $sliderDir . $sliderImgName);
    ...
    // set a name for the featured image and store it to local storage
    $featuredImgName = time() . '.' . $featuredImgExt;
    $featuredDir = public_path('assets/img/packages/');
  ...
  public function getSliderImages($id)
  {
    ...
    foreach ($sliderImages as $key => $sliderImage) {
      $data = asset('assets/img/packages/slider_images/' . $sliderImage);
  ...
  public function updatePackage(Request $request, $id)
  {
    ...
    if (!empty($packageSldImgs)) {
      foreach ($packageSldImgs as $key => $packageSldImg) {
        if (file_exists(public_path('assets/img/packages/slider_images/') . $packageSldImg)) {
          unlink(public_path('assets/img/packages/slider_images/') . $packageSldImg);
        }
      }
    }
    ...
    if ($request->filled('featured_img')) {
      ...
      if (
        ...
        file_exists(public_path('assets/img/packages/') . $package->featured_img)
      ) {
